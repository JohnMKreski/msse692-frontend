...
3. **Editor Component Test**:
   - Initialization triggers `listMine`; items set; count updates.
   - Error path sets `error` signal and renders message in template.
...
- Lightweight caching / memoization of last `listMine` response to avoid repeat fetches on quick navigations.
...
---
Migration complete; all owned-event views now rely on the backend endpoint.
