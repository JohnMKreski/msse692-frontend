...
describe('EventsService', () => {
  ...
  it('listPublicUpcoming should clamp limit and include from when provided', () => {
    ...
    const req = http.expectOne('/api/events/public-upcoming');
    expect(req.request.method).toBe('GET');
  ...
  it('get should GET /events/:id', () => {
    ...
    const req = http.expectOne('/api/events/123');
    expect(req.request.method).toBe('GET');
    req.flush({ id: 123 });
  });
  ...
  it('createRaw/updateRaw/delete should hit expected endpoints', () => {
    ...
    http.expectOne('/api/events').flush({ id: 1 });

    svc.updateRaw(1, { eventName: 'B' } as any).subscribe();
    http.expectOne('/api/events/1').flush({ id: 1 });

    svc.delete(1).subscribe();
    const del = http.expectOne('/api/events/1');
  ...
  it('audits and status transitions', () => {
    ...
    const auditsReq = http.expectOne(r => r.url === '/api/events/5/audits');
    expect(auditsReq.request.params.get('limit')).toBe('20');
    auditsReq.flush([]);

    svc.publishEvent(5).subscribe();
    http.expectOne('/api/events/5/publish').flush({ id: 5 });
    svc.unpublishEvent(5).subscribe();
    http.expectOne('/api/events/5/unpublish').flush({ id: 5 });
    svc.cancelEvent(5).subscribe();
    http.expectOne('/api/events/5/cancel').flush({ id: 5 });
  });
});
